...
export default function CatchTheNumberGame() {
    ...
    // Debug: Log whenever minNumber or maxNumber changes
    useEffect(() => {
        console.log(`📊 Number range updated: ${minNumber} to ${maxNumber}`);
    }, [minNumber, maxNumber]);
        ...
        const fetchLesson = async () => {
            try {
                ...
                if (data.lesson) {
                    ...
                    console.log('🎮 Game settings:', data.lesson.gameSettings);
                    
                    if (data.lesson.gameSettings?.numberRange) {
                        const min = data.lesson.gameSettings.numberRange.min || 1;
                        const max = data.lesson.gameSettings.numberRange.max || 10;
                        console.log(`✅ Setting number range: ${min} to ${max}`);
                        setMinNumber(min);
                        setMaxNumber(max);
                    } else {
                        console.warn('⚠️ No numberRange found in gameSettings, using defaults (1-10)');
                    }
                }
            } catch (error) {
    ...
    const generateRandomNumber = useCallback(() => {
        // range = max - min + 1 (10 dan 20 gacha = 11 ta raqam: 10,11,12...20)
        const range = maxNumber - minNumber + 1;
        let attempts = 0;
        let newNum;
        
        console.log(`🎲 Generating number between ${minNumber} and ${maxNumber}, range: ${range}`);
        
        // Agar range kichik bo'lsa (10 dan kam), oddiy random
        if (range <= 10) {
            do {
                // Math.random() * range → 0 dan range-1 gacha
                // minNumber + ... → minNumber dan maxNumber gacha
                const randomOffset = Math.floor(Math.random() * range);
                newNum = minNumber + randomOffset;
                console.log(`  ↳ Random offset: ${randomOffset}, Generated: ${newNum}`);
                attempts++;
        ...
        console.log(`✅ Final generated number: ${newNum} (min: ${minNumber}, max: ${maxNumber})`);
    ...
    const selectNextNumber = useCallback(() => {
        console.log(`🎯 Selecting next number with range: ${minNumber} to ${maxNumber}`);
        let newNum;
        do {
        ...
        console.log(`🎯 Selected target number:`, numObj);
    ...
    // O'yinni boshlash - faqat dars ma'lumotlari yuklanganidan keyin
    useEffect(() => {
        if (!gameStarted && !loading && lesson) {
            console.log('Starting game with settings:', { minNumber, maxNumber });
            if ('speechSynthesis' in window) {
    ...
    }, [gameStarted, selectNextNumber, loading, lesson, minNumber, maxNumber]);
    ...
    useEffect(() => {
        ...
        const spawnBalloon = () => {
            ...
            setBalloons((prevBalloons) => {
                ...
                console.log(`🎈 Spawning ${config.balloonCount} balloons (range: ${minNumber}-${maxNumber})`);
                ...
                for (let i = 0; i < config.balloonCount; i++) {
                    ...
                    if (!targetNumberSpawnedRef.current && timeSinceLastTarget >= MAX_TARGET_WAIT - config.spawnInterval) {
                        ...
                        console.log(`  🎯 Spawning TARGET balloon: ${balloonNumber}`);
                    } else {
                        balloonNumber = generateRandomNumber();
                        console.log(`  🎈 Spawning random balloon: ${balloonNumber}`);
    ...
    }, [gameActive, gameStarted, score, targetNumber, getSpawnConfig, getValidPosition, generateRandomNumber, minNumber, maxNumber]);
