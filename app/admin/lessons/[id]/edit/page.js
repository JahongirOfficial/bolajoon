...
export default function EditLessonPage() {
    ...
    const balloonColorOptions = [
        { name: 'Pushti', value: '#FF6B9D', gradient: 'linear-gradient(135deg, #FF6B9D, #FF8FAB)' },
        { name: 'Qizil', value: '#EF4444', gradient: 'linear-gradient(135deg, #EF4444, #F87171)' },
        { name: 'Sariq', value: '#FCD34D', gradient: 'linear-gradient(135deg, #FCD34D, #FDE68A)' },
        { name: 'Yashil', value: '#10B981', gradient: 'linear-gradient(135deg, #10B981, #34D399)' },
        { name: "Ko'k", value: '#3B82F6', gradient: 'linear-gradient(135deg, #3B82F6, #60A5FA)' },
        { name: 'Binafsha', value: '#A855F7', gradient: 'linear-gradient(135deg, #A855F7, #C084FC)' },
        { name: 'Apelsin', value: '#F97316', gradient: 'linear-gradient(135deg, #F97316, #FB923C)' },
        { name: 'Pushti-binafsha', value: '#EC4899', gradient: 'linear-gradient(135deg, #EC4899, #F472B6)' },
    ];
        ...
        try {
            ...
            if (data.success) {
                ...
                const normalizedGameSettings = {
                    ...
                    balloonColors: (() => {
                        const colors = lesson.gameSettings?.balloonColors || [];
                        // Check if colors are in new format (objects with hex and name)
                        if (colors.length > 0 && colors.every(c => c && typeof c === 'object' && c.hex && c.name)) {
                            return colors;
                        }
                        // Convert old format (hex strings) to new format
                        if (colors.length > 0 && colors.every(c => typeof c === 'string')) {
                            return colors.map((hex, index) => ({
                                hex: hex,
                                name: balloonColorOptions.find(opt => opt.value === hex)?.name || `Rang ${index + 1}`
                            }));
                        }
                        return [];
                    })()
                ...
                console.log('Fetched lesson gameSettings:', lesson.gameSettings);
                console.log('Normalized balloonColors:', normalizedGameSettings.balloonColors);
    ...
    return (
        <div style={{ maxWidth: '800px' }}>
            ...
            <form onSubmit={handleSubmit}>
                ...
                {/* Balloon Color Picker for pop-the-balloon game */}
                {formData.gameType === 'pop-the-balloon' && (
                    <div className="card border-0 rounded-4 shadow-sm mb-4">
                        <div className="card-body p-4">
                            <div className="mb-4">
                                <h3 className="h6 fw-bold mb-1 d-flex align-items-center gap-2">
                                    <span className="material-symbols-outlined" style={{ color: '#EC4899' }}>palette</span>
                                    Sharlar rangi
                                </h3>
                                <p className="text-muted small mb-0">
                                    O'yinda ishlatiladigan sharlar ranglarini tanlang va nom bering
                                </p>
                            </div>

                            {/* Predefined Colors */}
                            <div className="mb-4">
                                <label className="form-label fw-semibold small">Tayyor ranglar</label>
                                <div className="row g-3">
                                    {balloonColorOptions.map((color) => {
                                        const isSelected = formData.gameSettings.balloonColors.some(c => c.hex === color.value);
                                        return (
                                            <div key={color.value} className="col-6 col-md-3">
                                                <div
                                                    className={`card border-2 rounded-4 h-100 position-relative overflow-hidden ${
                                                    ...
                                                    style={{
                                                        ...
                                                        boxShadow: isSelected ? `0 8px 24px ${color.value}40` : 'none'
                                                    }}
                                                    onClick={() => {
                                                        setFormData(prev => ({
                                                            ...prev,
                                                            gameSettings: {
                                                                ...prev.gameSettings,
                                                                balloonColors: isSelected
                                                                    ? prev.gameSettings.balloonColors.filter(c => c.hex !== color.value)
                                                                    : [...prev.gameSettings.balloonColors, { hex: color.value, name: color.name }]
                                                            }
                                                        }));
                                                    }}
                                                >
                                                    ...
                                                    <div className="card-body p-3 text-center position-relative">
                                                        <div
                                                            ...
                                                            style={{
                                                                ...
                                                                height: '60px',
                                                                background: color.gradient,
                                                                boxShadow: `0 4px 12px ${color.value}30`
                                                            }}
                                                        >
                                                            <span style={{ fontSize: '32px' }}>🎈</span>
                                                        </div>

                                                        <p className="small fw-semibold mb-0" style={{ color: '#1d1d1f' }}>
                                                            {color.name}
                                                        </p>
                            ...
                            {/* Custom Color Adder */}
                            <div className="mb-4">
                                <label className="form-label fw-semibold small">Yangi rang qo'shish</label>
                                <div className="border rounded-3 p-3 bg-light">
                                    <div className="row g-3 align-items-end">
                                        <div className="col-md-4">
                                            <label className="form-label small">Rang tanlang</label>
                                            <input
                                                type="color"
                                                id="customColorPickerEdit"
                                        ...
                                        <div className="col-md-5">
                                            <label className="form-label small">Rang nomi</label>
                                            <input
                                                type="text"
                                                id="customColorNameEdit"
                                                className="form-control rounded-3"
                                        ...
                                        <div className="col-md-3">
                                            <button
                                                ...
                                                onClick={() => {
                                                    const colorInput = document.getElementById('customColorPickerEdit');
                                                    const nameInput = document.getElementById('customColorNameEdit');
                                                    ...
                                                    if (formData.gameSettings.balloonColors.some(c => c.hex === hex)) {
                                                        alert('Bu rang allaqachon qo\'shilgan!');
                                                        return;
                                                    }
                                                    ...
                                                    setFormData(prev => ({
                                                        ...
                                                        gameSettings: {
                                                            ...prev.gameSettings,
                                                            balloonColors: [...prev.gameSettings.balloonColors, { hex, name }]
                                                        }
                                                    }));
                            ...
                            {/* Selected Colors List with Name Edit */}
                            {formData.gameSettings.balloonColors.length > 0 && (
                                <div className="mt-4">
                                    <label className="form-label fw-semibold small mb-3">Tanlangan ranglar ({formData.gameSettings.balloonColors.length} ta)</label>
                                    <div className="d-flex flex-column gap-2">
                                        {formData.gameSettings.balloonColors.map((color, index) => (
                                            <div
                                                key={index}
                                                className="border rounded-3 p-3 bg-white d-flex align-items-center gap-3"
                                            >
                                                <div
                                                    ...
                                                    <input
                                                        ...
                                                        onChange={(e) => {
                                                            setFormData(prev => ({
                                                                ...
                                                                gameSettings: {
                                                                    ...prev.gameSettings,
                                                                    balloonColors: prev.gameSettings.balloonColors.map((c, i) =>
                                                ...
                                                <small className="text-muted" style={{ fontFamily: 'monospace', minWidth: '70px' }}>
                                                    {color.hex}
                                                </small>
                                                <button
                                                    type="button"
                                                    ...
                                                    onClick={() => {
                                                        setFormData(prev => ({
                                                            ...
                                                            gameSettings: {
                                                                ...prev.gameSettings,
                                                                balloonColors: prev.gameSettings.balloonColors.filter((_, i) => i !== index)
                                                            }
                                                        }));
                                                    }}
                                                >
                            ...
                            {/* Warning if no colors selected */}
                            {formData.gameSettings.balloonColors.length === 0 && (
                                <div className="alert alert-warning mt-4 mb-0 rounded-3">
                                    <small>
                                        <span className="material-symbols-outlined me-1" style={{ fontSize: '16px', verticalAlign: 'middle' }}>warning</span>
                                        <strong>Diqqat!</strong> Kamida bitta rang tanlang. Agar rang tanlanmasa, standart pushti rang ishlatiladi.
                                    </small>
                                </div>
                            )}
                        </div>
