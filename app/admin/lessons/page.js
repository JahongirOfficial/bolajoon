...
export default function AdminLessonsPage() {
    ...
    const { updateLesson, deleteLesson: deleteLessonFromCache } = useData();
    ...
    const fetchLessons = async () => {
        try {
            setLoading(true);
            // 🚀 Use admin endpoint with caching enabled
            const res = await fetch('/api/admin/lessons', {
                headers: getAuthHeader()
                // Removed cache: 'no-store' to enable caching
            });
                            ...
                            <div className="card-body p-0">
                                <div className="table-responsive">
                                    <table className="table table-hover mb-0">
                                        ...
                                        <tbody>
                                            {levelLessons.sort((a, b) => (a.order || 0) - (b.order || 0)).map((lesson, idx) => (
                                                <tr 
                                                    key={lesson._id}
                                                    data-aos="fade-up"
