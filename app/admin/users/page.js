    ...
    const handleActivateSubscription = async () => {
        ...
        if (selectedDays < 0) {
            setAlertModal({
                show: true,
                title: 'Xatolik',
                message: 'Kunlar soni manfiy bo\'lishi mumkin emas',
                type: 'danger'
            });
            return;
        }
        ...
        try {
            ...
            if (data.success) {
                ...
                setAlertModal({
                    show: true,
                    title: 'Xatolik',
                    message: data.error || 'Obunani o\'zgartirishda xatolik',
                    type: 'danger'
                });
            }
        } catch (error) {
            ...
            setAlertModal({
                show: true,
                title: 'Xatolik',
                message: 'Obunani o\'zgartirishda xatolik',
                type: 'danger'
            });
        } finally {
                        ...
                        <div className="modal-content rounded-4 border-0">
                            ...
                            <div className="modal-body">
                                <p className="text-muted mb-3">
                                    <strong>{subscriptionModal.user?.name}</strong> ning obuna muddatini o'zgartirish:
                                </p>

                                {/* Current Days */}
                                <div className="bg-light rounded-3 p-3 mb-4 text-center">
                                    <p className="small text-muted mb-1">Joriy obuna</p>
                                    <h4 className="fw-bold text-success mb-0">
                                        {subscriptionModal.user?.daysRemaining || 0} kun
                                    </h4>
                                </div>
                                ...
                                <div className="mb-4">
                                    ...
                                    <div className="input-group">
                                        <input
                                            ...
                                            value={selectedDays}
                                            onChange={(e) => setSelectedDays(Math.max(0, parseInt(e.target.value) || 0))}
                                            min={0}
                                            style={{ fontSize: '24px' }}
                                        />
                                ...
                                <div className="d-flex flex-wrap gap-2">
                                    {[0, 1, 3, 7, 15, 20, 30, 60, 90, 180, 365].map(days => (
                                        <button
                                            key={days}
                                            onClick={() => setSelectedDays(days)}
                                            className={`btn btn-sm rounded-pill px-3 ${selectedDays === days ? 'btn-success' : 'btn-outline-secondary'}`}
                                        >
                                            {days} kun
                                        </button>
                                    ))}
                            ...
                            <div className="modal-footer border-0 pt-0">
                                ...
                                <button
                                    type="button"
                                    className={`btn rounded-3 d-flex align-items-center gap-2 ${selectedDays === 0 ? 'btn-danger' : 'btn-primary'}`}
                                ...
                                >
                                    ...
                                    {selectedDays === 0 ? 'Obunani to\'xtatish' : `${selectedDays} kun faollashtirish`}
                                </button>
                            </div>
                        </div>
                    </div>
