...
export function DataProvider({ children }) {
    ...
    const loadAllData = useCallback(async () => {
        ...
        // Set 4 second timeout for critical data
        ...
        try {
            ...
            const [lessonsData, studentsData, rewardsData, dashboardData] = await Promise.all([
            ...
            // Step 2: Fetch statistics lazily in the background (non-blocking)
            fetch('/api/statistics', { headers })
                .then(res => res.ok ? res.json() : { success: false })
                .then(data => { if (data.success) setStatistics(data); })
                .catch(() => {});
