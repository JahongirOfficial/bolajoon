...
// Send JSON backup as document
export async function sendBackupJson(buffer, fileName, counts) {
  try {
    const telegramBot = initBot();
    if (!telegramBot) return false;

    const caption = `🗄 <b>SOATLIK BACKUP</b>\n⏰ ${new Date().toLocaleString('uz-UZ', { timeZone: 'Asia/Tashkent' })}\n\n👥 Foydalanuvchilar: ${counts.users}\n🎓 O'quvchilar: ${counts.students}\n📚 Darslar: ${counts.lessons}\n📊 Progress: ${counts.progress}`;
    ...
    console.error('Backup yuborishda xato:', error);
    return false;
  }
}

// Payment receipt notification to admin
export async function sendReceiptNotification(user, days, amount, imageBuffer) {
  try {
    const telegramBot = initBot();
    if (!telegramBot) return false;
...
📅 <b>So'ralgan kunlar:</b> ${days} kun
💰 <b>Summa:</b> ${amount.toLocaleString()} so'm
⏰ <b>Vaqt:</b> ${new Date().toLocaleString('uz-UZ', { timeZone: 'Asia/Tashkent' })}

⚠️ Admin paneldan tekshirib kun qo'shing!
    `.trim();

    await telegramBot.sendPhoto(TELEGRAM_CHAT_ID, imageBuffer, {
      caption,
      parse_mode: 'HTML'
    });
    return true;
